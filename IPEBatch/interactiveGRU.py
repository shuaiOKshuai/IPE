...
import interactiveGRULearningBatch
...
def adadelta(lr, tparams, grads, trainingParis, sequences_matrix, dependency_matrix, dependWeight_matrix, sequencesLen_vector, discountSeq_matrix, discountForEachNode_matrix, wordsEmbs, typesEmbs, masks_matrix, groups_tensor, cost):
    """
    ...
    f_grad_shared = theano.function([trainingParis, sequences_matrix, dependency_matrix, dependWeight_matrix, sequencesLen_vector, discountSeq_matrix, discountForEachNode_matrix, wordsEmbs, typesEmbs, masks_matrix, groups_tensor], cost, updates=zgup + rg2up,
...
def init_sharedVariables(options):
    ...
    params = OrderedDict()
    Q_1=init_params_weight(options['dimension'], options['attention_dimension'])
    Q_2=init_params_weight(options['type_dimension'], options['attention_dimension'])
    Q_b=numpy.random.rand(options['attention_dimension'], ).astype(config.floatX)-0.5  # @UndefinedVariable # 
    eta=numpy.random.rand(options['attention_dimension'], ).astype(config.floatX)-0.5  # @UndefinedVariable # 
    W_z=init_params_weight(options['word_dimension'],options['dimension'])
    W_r=init_params_weight(options['word_dimension'],options['dimension'])
    W_h=init_params_weight(options['word_dimension'],options['dimension'])
...
def interactiveGRUTraining(
    ...
    trainingParis, sequences_matrix, dependency_matrix, dependWeight_matrix, sequencesLen_vector, discountSeq_matrix, discountForEachNode_matrix, wordsEmbs, typesEmbs, masks_matrix, groups_tensor, cost=interactiveGRULearningBatch.interactiveGRULearning(model_options, tparams)
    
    print 'Generate gradients ......'
    grads=tensor.grad(cost,wrt=list(tparams.values()))
    print 'Using Adadelta to generate functions ......'
    lr = tensor.scalar(name='lr')
    f_grad_shared, f_update=adadelta(lr, tparams, grads, trainingParis, sequences_matrix, dependency_matrix, dependWeight_matrix, sequencesLen_vector, discountSeq_matrix, discountForEachNode_matrix, wordsEmbs, typesEmbs, masks_matrix, groups_tensor, cost)
    ...
    for eidx in range(max_epochs):
        for _, batch in allBatches: 
            ...
            if saveto and numpy.mod(uidx, saveFreq) == 0:
                print 'Saving... time ==',time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
